...
core_time_power core_time_power_info[SIZE];
...
int bin_to_decimal(int binary)
{
	...
	{
        	remainder = binary % 10;
        	decimal = decimal + remainder * mult;
        	mult = mult * 2;
        	binary=binary/10;
    	}	
	return decimal;
...
long int bin_packing(int *particle_info, int index)
{
	int binary;
	...
	for(int i = 0; i< SIZE; i++)
	{
		char power_file[100] = "power_value_";
		char a[10];
		sprintf(a,"%d",i+1);
		strcat(power_file, a);
	...
	while (no_of_core_scheduled < SIZE)
	{
		...
		while(flag ==1)
		{
			...
			for (int kk = 0; kk< SIZE; kk++)
			{
				if(schedule_index[kk] == 0 )
				{
					if (available_tam_width[jj] >= assigned_tam[kk] )
					{
						...
						//printf("\n\n**************\n\n");
						//printf("\ntemp check %d\t%ld\n\n", temp_check_time, running_power_time_info.time_info[temp_latest_power_index]);
						while(core_power_index < core_time_power_info[kk].no_of_instans)
						{
							...
							{	
								...
								//printf("\n in 1st while %ld", temp_latest_power_index);
							}
							//printf("\ntemp check***** %d\t%ld\n\n", temp_check_time + core_time_power_info[kk].time_instant[core_power_index], running_power_time_info.time_info[temp_latest_power_index]);
							...
							{
								//printf("\n\n 3rd if\n");
								//printf("\ntemp check* inside 3rd %d\t%ld\t%ld\n\n", temp_check_time + core_time_power_info[kk].time_instant[core_power_index], running_power_time_info.time_info[temp_latest_power_index],running_power_time_info.time_info[temp_latest_power_index +1]);
								if(running_power_time_info.power_info[temp_latest_power_index] + core_time_power_info[kk].power_instant[core_power_index+1] > power_budget )
								{
										//printf("\n\n 3rd if brk\n\n");
									check_flag = 0;
								...
								//printf("\ntemp check* inside 3rd aftr if %d\t%ld\t%ld\n\n", temp_check_time + core_time_power_info[kk].time_instant[core_power_index], running_power_time_info.time_info[temp_latest_power_index],running_power_time_info.time_info[temp_latest_power_index +1]);
								if(check_flag == 0)
									break;
								else	
									core_power_index++;
			...
			//for(int mm = 0; mm < MAX_NO_OF_INTERVAL; mm++)
				//printf("%ld\t", running_power_time_info.time_info[mm]);
		...
		//for(int mm = 0; mm < MAX_NO_OF_INTERVAL; mm++)
		//	printf("%lf\t", running_power_time_info.power_info[mm]);
/*		
		
		int latest_schedule_cores[SIZE];
		long int latest_schedule_core_testtime[SIZE];
		double latest_schedule_core_power[SIZE];
	...
	//{
		...
		{	
			int kk = 0;
			while((no_of_latest_scheduled_core - core_check) > 0)
			{		
...
/*				for (int i = 0; i< SIZE; i++)
				...
				if(check == 1)
				{
					...
					//next_power = tam_testtime[core_number].peak_power_list[next_tam_index-1]; 
...
/*					scheduler [index][core_index_in_scheduler].tam_width = next_tam;
					scheduler [index][core_index_in_scheduler].power = next_power;
...
int particle_swarm_optimization()
{
	...
	for (int i=0;i<NO_OF_PARTICLES;i++)
    	{
    		partarray[i].time_fitness = bin_packing(partarray[i].info,i);  //testtime returns maxtime for scheduling process
    		//printf("\n\n *********my current position*********\n\n");
    		//printf("\n\n*********particle no %d*********\n\n", i);
    	...
    	while(gencount<MAX_ITERATION)
    	{
        	...
        	for(int i=0;i<NO_OF_PARTICLES;i++)
        	{
    			...
    			partarray[i].time_fitness = bin_packing(partarray[i].info,i);
...
int main(int argc, char *argv [ ])
{
	...
	//TAM_WIDTH_MAX = atoi(argv[4]);
	//TIME_INTERVAL = atoi (argv[5]);
	...
	for(int ii = 0; ii< SIZE; ii++)
	{
		...
		{
			fscanf(tamread,"%d\t%ld\n", &tam_testtime[ii].tam_list[jj], &tam_testtime[ii].testtime_list[jj]/*, &tam_testtime[ii].peak_power_list[jj]*/); 
		}
	}
	fclose (tamread);
